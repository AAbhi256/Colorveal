...
// ck.setAttribute("id", "check")
// ck.addEventListener("change", checkHandler);
// //.textContent = "See As Colorblind";
//  ul.appendChild(ck);
...
function checkHandler(){
  ...
  if(!ck.checked){
    ...
    //   chrome.tabs.sendMessage(tabs[0].id, { msg: "add", data: null }, (response) => {
    //   });
    // });
  }
}
function handler(e) {
  ...
  chrome.tabs.insertCSS(null, { code: 'html { -webkit-filter: url(#' + current +checked+ '); }' });
...
function handler2a(e) {
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    ...
    let sfilter = current + "Matrix" +checked;
